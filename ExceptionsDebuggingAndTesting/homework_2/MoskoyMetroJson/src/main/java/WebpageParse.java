...
public class WebpageParse {
    ...
    public List<Line> getLines() throws Exception {
        ...
        //затем с помощью forEach перебираем нашу кол-цию и вносим в список все эти ссылки и text() дает название линии и станции
    ...
    public List<Station> getStation() throws Exception {
        try {
            ...
            Elements elements1 = doc1.select("span.name");
            elements1.forEach(element -> {
                //stations.add(new Station(element.text(), new Line(element.attr("data-line"), element.text())));
                stations.add(new Station(element.parents().attr("data-line"), element.text()));
            });
